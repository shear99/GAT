...
PREDICTED_JSON_DIR = './output/predict_json'   # 예측 결과 JSON 저장 경로
...
def plot_and_save(ax, original_keypoints, predicted_keypoints, x_min, y_min, width_bbox, height_bbox, similarity_score, filename, image_width, image_height, distances, mean_distance):
    ...
    ax.axis('off')
    original_path = os.path.join(ORIGINAL_IMG_DIR, f"{filename}_original.png")                         #############                     
    ...
    ax.axis('off')
    eval_path = os.path.join(EVAL_IMG_DIR, f"{filename}_eval.png")                                      ##########
    ...
    ax.legend()
    ax.axis('off')
    diff_path = os.path.join(DIFF_IMG_DIR, f"{filename}_differences.png")
